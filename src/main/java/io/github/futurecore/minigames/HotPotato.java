...
public class HotPotato implements Listener {
    ...
    private void startEliminationTimer () {
        ...
        eliminationTask.runTaskLater ( Main.instance, 20 * 20L );
    }
    ...
    public void onDrop ( PlayerDropItemEvent event ) {
        if (!gameRunning || potatoHolder == null) return;
        if (event.getPlayer ( ).getInventory ( ).getItemInHand ( ).getType ( ).getId ( ) != 393) return;
        if (!RegionUtils.isLocationInRegion ( event.getPlayer ( ).getLocation ( ), regionName )) return;
        event.setCancelled ( true );
        event.getPlayer ( ).sendMessage ( CC.translate ( "&4Debes golpear al jugador para pasar este item" ) );
    }
